...
- Dashboard App:
  ...
  - HealthDataScreen ✅ — מציג HR ו-steps בזמן אמת מ-`GET /users/{id}/health-metrics/latest` (עם `useFocusEffect`). גרף חודשי נגזר מ-`GET /users/{id}/health-metrics/history?days=30` דרך `aggregateHistoryByDay` (קיבוץ יומי: דופק=ממוצע, צעדים=מקסימום) + `sampleEvenly` (עד 6 נקודות לתצוגה) — **בלי mock data** (MOCK_DATA/LAST_MONITORING הוסרו לגמרי). כרטיס "מדדים חריגים" מציג "צעדים" רק כשבאמת מחוץ לטווח 3,000–8,000 (לא unconditionally). ייצוא PDF **מומש** (`expo-print`+`expo-sharing`, HTML נבנה ב-`buildHealthSummaryHtml`, לוגו מוטמע כ-base64 מ-`ui/assets/anchorLogoBase64.js`) — **⚠️ באג פתוח**: טבלת ההיסטוריה (עד 30 שורות) נחתכת באמצע שורה כשחוצה עמוד ב-PDF (WebKit/iOS לא אוכף `page-break-inside:avoid` על `<tr>` באמינות; מבנה `<div>`/flexbox הוא הפתרון הנכון אך טרם אומת בפועל).
...
- **Push notifications** — FCM לשעון: `medication_sync` (סנכרון תרופות אחרי יצירה), `medication_delete` (ביטול אלרם ספציפי אחרי מחיקה — כולל `med_id`), `water_sync` (סנכרון תזכורות מים אחרי שמירת הגדרות — אין `water_delete` נפרד), `request_checkin` (פותח `CheckInActivity`), `watch_unpair` (מנקה `WatchKeyStore` ומפעיל מחדש את `MainActivity` — נוחת על מסך הקישור). Expo push לדאשבורד: `emergency` (SOS/נפילה), `medication_taken` (אישור נטילת תרופה עם שם התרופה). **לא** נשלח push על אישור שתיית מים — הוחלט במפורש שזה לא alert-worthy.
- **watch_unpair — טריגר יחיד, לא 3 שכבות** — בניגוד ל-`request_checkin` (WakeLock+startActivity ישיר → `setAlarmClock(now)` → fullScreenIntent, מיועד כ-fallbacks חופפים שרק אחד מהם אמור להצליח בפועל), הטיפול ב-`watch_unpair` ב-`WatchFcmService` משתמש **רק** ב-`AlarmManager.setAlarmClock()`. הסיבה: `request_checkin`/`medication`/`water`/`FallAlert` מכוונים לActivities עם `launchMode="singleInstance"` — הפעלה כפולה פשוט מביאה את אותו instance קדימה. `MainActivity` (יעד ה-`watch_unpair`) היא ה-launcher עם launchMode ברירת מחדל (`standard`) — ומכיוון ש-`FallDetectionService` שומר את הprocess חי כמעט תמיד, direct `startActivity()` כמעט תמיד מצליח **גם הוא וגם** ה-alarm, ומפעיל את `MainActivity` פעמיים ברצף (`CLEAR_TASK` ⇒ `onCreate()` חדש בכל פעם). בדיקה בפועל חשפה שזה יוצר שתי רשומות `type:"pairing"` יתומות ב-`Anchor_Users` (כי `WatchPairingScreen`'s `LaunchedEffect(Unit)` קורא `initPairing()` בכל רכיבה). כל טריגר-רקע עתידי שמכוון ל-`MainActivity` עצמה (ולא לאחד מ-4 מסכי ה-singleInstance) חייב טריגר יחיד.
- **TTL מכובה על `Anchor_Users`** — רשומות `type:"pairing"` הזמניות (מ-`watch-init-pairing`, עם שדה `ttl`) לא נמחקות אוטומטית ע"י DynamoDB; הפג-תוקף נאכף רק לוגית (`expires_at` ב-`watch-pair.js`). pairing נטוש (לא הושלם, או שנוצר כפול) נשאר בטבלה לצמיתות. פער ידוע, לא תוקן.
...
- Watch: POST /users/{id}/watch/unpair (JWT) — best-effort FCM `watch_unpair` לטוקן הנוכחי, ואז מסיר watch_id/watch_api_key/watch_paired_at/watch_name/watch_fcm_token מ-`Anchor_Users` (MVP_TODO 1.4) ✅
...
- Health metrics: POST /health-metrics (X-Watch-Key, כותב ל-`Anchor_BiometricData`), GET /users/{id}/health-metrics/latest (JWT), GET /users/{id}/health-metrics/history?days=30 (JWT — מחזיר קריאות גולמיות מ-N הימים האחרונים, `#ts >= :start` כי `timestamp` שמור ב-DynamoDB) ✅
