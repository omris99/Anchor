...
**החלטה:**
- Auth (login, register, MFA) — **אפליקציית הדאשבורד בלבד** (בני משפחה + קשישים שמתחברים דרך הנייד)
- השעון **לא עובר** flow של username/password — הוא מזדהה עם **API Key** (ראה סעיף 3)

---

## 2. קישור שעון לחשבון קשיש (Watch Pairing)

**עמימות:** ה-PRD ציין QR code אך לא הסביר מה ה-QR מכיל ומי סורק אותו.

**החלטה:**
- השעון מציג QR code על המסך
- **הקשיש** מתחבר לאפליקציית הדאשבורד בחשבונו ולוחץ "קשר שעון"
...
- **Flow טכני:**
  ...
  4. Backend מאמת את ה-token ומקשר בין watch_id לחשבון הקשיש
...
## 4. קישור בני משפחה לקשיש (Family Member Linking)
...
- Flow:
  1. בן משפחה מזין מספר הטלפון של הקשיש בדאשבורד
  2. Backend שולח בקשת קישור לקשיש
  3. **הקשיש מאשר** (או דוחה) דרך אפליקציית הדאשבורד שלו
...
- **בני משפחה / קשיש** יוצרים תזכורות דרך הדאשבורד → נשמרות ב-DynamoDB
